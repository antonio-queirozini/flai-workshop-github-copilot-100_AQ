...
def test_signup_and_unregister():
    ...
    assert unique_email in activities.get("Drama Club", {}).get("participants", [])

    # Unregister should succeed for a registered participant
    ...
    activities = client.get("/activities").json()
    assert unique_email not in activities.get("Drama Club", {}).get("participants", [])
